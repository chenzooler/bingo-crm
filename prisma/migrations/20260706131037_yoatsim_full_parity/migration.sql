...
CREATE TABLE "SentForm" (
    ...
    "signedAt" DATETIME,
    CONSTRAINT "SentForm_leadId_fkey" FOREIGN KEY ("leadId") REFERENCES "Lead" ("id") ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE "new_Lead" (
    ...
    "extraJson" TEXT,
    "cardKind" TEXT NOT NULL DEFAULT 'card',
    ...
    CONSTRAINT "Lead_parentLeadId_fkey" FOREIGN KEY ("parentLeadId") REFERENCES "Lead" ("id") ON DELETE SET NULL ON UPDATE CASCADE,
...
INSERT INTO "new_Lead" ("accountRestricted", "additionalIncome", "address", "amountRequested", "attemptsCount", "bankAccount", "bankBranch", "bankCode", "bankName", "bdiApproved", "birthDate", "cardLimit", "category", "city", "createdAt", "creditCardsJson", "email", "employerName", "employmentStatus", "existingLoansJson", "exitReason", "externalId", "extraJson", "feeAmount", "feePaid", "finalApprovedAmount", "finalInterest", "finalLenderKey", "finalMonths", "firstName", "fullName", "gender", "hadCreditIssues", "hadEnforcement", "hasProperty", "hasVehicle", "houseNumber", "housing", "id", "idNumber", "importBatchId", "intakeDate", "journeyJson", "journeyUpdatedAt", "journeyVersion", "lastName", "loanPurpose", "maritalStatus", "monthlyHousingPayment", "monthlyIncome", "monthlyObligations", "numberOfChildren", "ownerId", "phone", "phone2", "pipelineKey", "providerId", "seniorityMonths", "smiley", "source", "sourceText", "spouseIncome", "stage", "stageChangedAt", "statusKey", "syncSource", "updatedAt", "vehicleMake", "vehicleYear", "zip") SELECT "accountRestricted", "additionalIncome", "address", "amountRequested", "attemptsCount", "bankAccount", "bankBranch", "bankCode", "bankName", "bdiApproved", "birthDate", "cardLimit", "category", "city", "createdAt", "creditCardsJson", "email", "employerName", "employmentStatus", "existingLoansJson", "exitReason", "externalId", "extraJson", "feeAmount", "feePaid", "finalApprovedAmount", "finalInterest", "finalLenderKey", "finalMonths", "firstName", "fullName", "gender", "hadCreditIssues", "hadEnforcement", "hasProperty", "hasVehicle", "houseNumber", "housing", "id", "idNumber", "importBatchId", "intakeDate", "journeyJson", "journeyUpdatedAt", "journeyVersion", "lastName", "loanPurpose", "maritalStatus", "monthlyHousingPayment", "monthlyIncome", "monthlyObligations", "numberOfChildren", "ownerId", "phone", "phone2", "pipelineKey", "providerId", "seniorityMonths", "smiley", "source", "sourceText", "spouseIncome", "stage", "stageChangedAt", "statusKey", "syncSource", "updatedAt", "vehicleMake", "vehicleYear", "zip" FROM "Lead";
DROP TABLE "Lead";
...
-- CreateIndex
CREATE INDEX "SentForm_leadId_idx" ON "SentForm"("leadId");
